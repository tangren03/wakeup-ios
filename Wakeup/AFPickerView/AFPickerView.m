...
- (id)initWithFrame:(CGRect)frame
{
    ...
    if (self) 
    {
        ...
        contentView.backgroundColor = [Utils colorWithHexString:MAIN_COLOR];
        ...
        // glass
        glassImageView = [[UIImageView alloc] initWithFrame:CGRectMake(0.0, 0.0, self.frame.size.width, self.frame.size.height / 3)];
        glassImageView.center = contentView.center;
        glassImageView.backgroundColor = [Utils colorWithHexString:YELLOW_COLOR];
        glassImageView.alpha = 0.6;
...
- (void)reloadData
{
    ...
    contentView.contentSize = CGSizeMake(contentView.frame.size.width, 39.0 * rowsCount + 4 * (self.frame.size.height / 3));
    [self tileViews];
}
...
- (void)determineCurrentRow
{
    ...
    int position = round(delta / (self.frame.size.height / 3));
    currentRow = position;
    [contentView setContentOffset:CGPointMake(0.0, (self.frame.size.height / 3) * position) animated:YES];
...
- (void)didTap:(id)sender
{
    ...
    int steps = floor(point.y / (self.frame.size.height / 3));
...
- (void)makeSteps:(int)steps
{
    ...
    [contentView setContentOffset:CGPointMake(0.0, (self.frame.size.height / 3) * currentRow) animated:NO];
    ...
    [contentView setContentOffset:CGPointMake(0.0, (self.frame.size.height / 3) * currentRow) animated:YES];
    [delegate pickerView:self didSelectRow:currentRow];
}
...
- (BOOL)isDisplayingViewForIndex:(NSUInteger)index
{
    ...
    for (UIView *aView in visibleViews) 
	{
        int viewIndex = aView.frame.origin.y /(self.frame.size.height / 3) - 2;
        if (viewIndex == index) 
		{
            foundPage = YES;
            break;
...
- (void)tileViews
{
    ...
    int firstNeededViewIndex = floorf(CGRectGetMinY(visibleBounds) / (self.frame.size.height / 3));
    int lastNeededViewIndex  = floorf((CGRectGetMaxY(visibleBounds) / (self.frame.size.height / 3)));
	...
	for (UIView *aView in visibleViews) 
    {
        int viewIndex = aView.frame.origin.y / (self.frame.size.height / 3);
	...
	for (int index = firstNeededViewIndex; index <= lastNeededViewIndex; index++) 
	{
        if (![self isDisplayingViewForIndex:index]) 
		{
			...
			if (label == nil)
            {
				label = [[UILabel alloc] initWithFrame:CGRectMake(0, 0, self.frame.size.width, self.frame.size.height / 3)];
...
- (void)configureView:(UIView *)view atIndex:(NSUInteger)index
{
    ...
    frame.origin.y = (self.frame.size.height / 3) * index + (self.frame.size.height / 3);
    label.frame = frame;
}
